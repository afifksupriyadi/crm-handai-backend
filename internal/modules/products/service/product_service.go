...
type ProductServiceImpl struct {
	productRepo repository.ProductRepository // ✅ ONLY repo!
}
...
func (s *ProductServiceImpl) GetProductByID(ctx context.Context, id int) (*model.Product, error) {
	...
	if product == nil {
		return nil, response.WrapAppError(ctx, nil, response.ErrProductNotFound, "Product not found")
	}
	return product, nil
}
...
func (s *ProductServiceImpl) GetOrCreateProduct(ctx context.Context, product *model.Product) (*model.Product, error) {
	...
	logger.FromContext(ctx, 1).Info().Int("product_id", product.ID).Str("name", product.Name).Msg("Product created")
	return product, nil
}
...
func (s *ProductServiceImpl) GetOrCreateProductInTx(ctx context.Context, tx *bun.Tx, product *model.Product) (*model.Product, error) {
	...
	logger.FromContext(ctx, 1).Debug().Int("product_id", product.ID).Str("name", product.Name).Msg("Product created in transaction")
	return product, nil
}

...
func RegisterCustomerRoutes(api huma.API, h *handler.CustomerHandler) {
	...
	// GET /api/customers/recent-transactions - Get customers with recent transactions
	huma.Register(api,
		huma.Operation{
			...
			Description: "Get all customers who have made transactions with their latest metrics from analytics, ordered by most recent transaction date.",
			Tags:        []string{"customers"},
			Security: []map[string][]string{
				{"bearerAuth": {}},
			},
